...
public:
    ...
    void Filtrar_Nombre(string buscar)
    {
        ...
        while (recorrido != NULL)
        {
            temp = recorrido->getDato().getNombre();
            ...
            if (buscar.length() == 1)
            {
                ...
                for (int i = 0; i < temp.length(); i++)
                {
                    if (temp[i] == buscar[k] && temp[i + 1] == buscar[k + 1])
                    {

                        if (k + 1 == buscar.length() - 1)
                        {
                            recorrido->imprimir();
                            encontrado = true;
                            break;
...
void MostrarMenu()
{
    ...
    cout << "9.- Filtrar por nombre" << endl;
    cout << "0.- Salir" << endl;
}
